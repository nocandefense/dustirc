...
suite('Extension Integration Tests', () => {
    ...
    test('IRC connection events trigger proper UI updates', async () => {
        // Test simulating IRC events and checking UI responses
        // Since the connection is in a closure, we'll test the command flows that trigger events

        // Test connect flow that sets up event handlers
        ...
        assert.ok(fakeVscode.window.showInformationMessage.called, 'Should show connection message');
    });
    ...
    test('Complex connection flow with all options', async () => {
        // Test full connection flow with TLS and NickServ
        fakeVscode.window.showInputBox = sinon.stub()
            .onCall(0).resolves('irc.libera.chat')
        ...
        // Should handle full connection flow with TLS and NickServ
        assert.ok(fakeVscode.window.showInformationMessage.called, 'Should show connection success with TLS');
    });
